...
def load_cologne_list():
    try:
        ...
        # Combines Brand and your new 'Perfume' header
...
with col2:
    ...
    # NEW: Fragrance Type Selector
    frag_type = st.selectbox("Fragrance Type", ["Any", "Designer", "Niche", "Clone / Inspiration"])
...
if st.button("Find My Match & Best Prices", use_container_width=True):
    ...
    - Budget Range: {budget}
    - Preferred Category: {frag_type}
    
    Choose ONE from this list: {cologne_list}
    
    RULES:
    1. If 'Clone' is selected, prioritize brands known for inspirations (like Lattafa, Maison Alhambra, Dossier).
    2. If 'Niche' is selected, prioritize high-end artistry (like Creed, Xerjoff, Parfums de Marly).
    3. If 'Designer' is selected, look for major fashion houses (like Chanel, Dior, Armani).
    4. The recommendation MUST fit the budget.
    
    Line 1: EXACT NAME ONLY.
    Line 2+: Stylish explanation of why it fits the vibe and category.
    """
    ...
    with st.spinner("Curating your match..."):
        try:
            ...
            st.error(f"Error: {e}")            for result in data["shopping_results"][:3]:
                price = result.get("price")
                source = result.get("source")
                link = result.get("link") or result.get("product_link")
                if price and source and link:
